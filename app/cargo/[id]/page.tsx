...
export default function CargoDetailPage() {
  ...
  return (
    <div style={{minHeight:"100vh",fontFamily:"Vazirmatn,sans-serif",direction:"rtl",background:"#f4f6f9",color:"#333"}}>
      <Navbar />
      <main style={{maxWidth:"720px",margin:"0 auto",padding:"32px 20px"}} className="main-content">
        ...
        <div className="animate-fade" style={{background:"white",padding:"28px",borderRadius:"20px",border:"1px solid #eee",boxShadow:"0 4px 20px rgba(0,0,0,0.06)",marginBottom:"20px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"20px",paddingBottom:"16px",borderBottom:"2px solid #f0f4ff",flexWrap:"wrap",gap:"8px"}}>
          ...
          <div style={{marginBottom:"20px"}}><RouteMap origin={cargo.origin_city} destination={cargo.dest_city} height="280px" /></div>
          <div className="detail-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"20px"}}>
            {[{l:"نوع خودرو",v:VL[cargo.vehicle_type_needed]||cargo.vehicle_type_needed,i:"🚛",bg:"#ecfeff"},{l:"وزن",v:cargo.weight_tons?cargo.weight_tons+" تن":"نامشخص",i:"⚖️",bg:"#fffbeb"},{l:"تاریخ بارگیری",v:cargo.pickup_date,i:"📅",bg:"#ecfdf5"},{l:"وضعیت",v:cargo.status==="open"?"باز":"در حال حمل",i:"📊",bg:"#f0f4ff"}].map((x,i)=>(
              <div key={i} style={{background:x.bg,padding:"14px",borderRadius:"12px",display:"flex",alignItems:"center",gap:"10px"}}>
                <span style={{fontSize:"20px"}}>{x.i}</span>
                <div><div style={{fontSize:"11px",color:"#888",fontWeight:900}}>{x.l}</div><div style={{fontSize:"14px",fontWeight:900,color:"#1e3a5f"}}>{x.v}</div></div>
              </div>
            ))}
          </div>
          {cargo.cargo_description && <div style={{background:"#f8fafc",padding:"14px",borderRadius:"12px",marginBottom:"20px"}}><div style={{fontSize:"11px",color:"#888",fontWeight:900,marginBottom:"4px"}}>توضیحات</div><div style={{fontSize:"14px",color:"#333",fontWeight:700,lineHeight:"1.8"}}>{cargo.cargo_description}</div></div>}
          <div style={{background:"linear-gradient(135deg,#ecfeff,#e0f2fe)",padding:"20px",borderRadius:"14px",textAlign:"center",border:"2px solid #06b6d422"}}><div style={{fontSize:"13px",color:"#0e7490",fontWeight:900,marginBottom:"4px"}}>قیمت پیشنهادی</div><div style={{fontSize:"30px",fontWeight:900,color:"#0ea5e9"}}>{formatPrice(cargo.price_suggestion)}</div></div>
        </div>
        {isOwner ? (
          <div className="animate-fade-up" style={{background:"white",padding:"28px",borderRadius:"20px",border:"1px solid #eee",boxShadow:"0 4px 20px rgba(0,0,0,0.06)"}}>
            ...
            {bookings.length === 0 ? <div style={{textAlign:"center",padding:"36px",color:"#999"}}><div style={{fontSize:"36px",marginBottom:"8px"}}>⏳</div><p style={{fontWeight:700}}>هنوز کسی درخواست نداده</p></div> :
              bookings.map(b => (
          ...
          <div className="animate-fade-up" style={{background:"white",padding:"28px",borderRadius:"20px",border:"1px solid #eee",boxShadow:"0 4px 20px rgba(0,0,0,0.06)"}}>
            ...
            {success ? (
              <div style={{textAlign:"center",padding:"36px"}} className="animate-scale">
                ...
                <p style={{color:"#888",fontSize:"14px",fontWeight:700}}>بارفرست بررسی می‌کنه</p>
              ...
              <>
                <div style={{marginBottom:"20px"}}><label style={{display:"block",marginBottom:"6px",fontSize:"13px",fontWeight:900,color:"#444"}}>قیمت پیشنهادی (تومان)</label><input type="number" value={bookingPrice} onChange={e=>setBookingPrice(e.target.value)} placeholder="مثلاً ۱۲,۰۰۰,۰۰۰" dir="ltr" className="input-field" /></div>
                <div style={{marginBottom:"22px"}}><label style={{display:"block",marginBottom:"6px",fontSize:"13px",fontWeight:900,color:"#444"}}>پیام به بارفرست</label><textarea value={bookingMsg} onChange={e=>setBookingMsg(e.target.value)} placeholder="مثلاً: ناوگان آماده‌ست" className="input-field" style={{minHeight:"90px"}} /></div>
