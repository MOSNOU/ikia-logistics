...
export default async function SupplierQuotationsPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">پیشنهادهای قیمت</h1>
          <p className="text-sm text-muted-foreground">
            پیشنهادهای ارسال‌شده به خریداران — وضعیت، اعتبار و پاسخ آن‌ها.
          </p>
        </div>
        <Button asChild>
          <Link href="/supplier/quotations/new">ایجاد پیشنهاد جدید</Link>
        </Button>
      </div>
      ...
      {rows.length === 0 ? (
        <TableEmpty>هیچ پیشنهادی یافت نشد.</TableEmpty>
      ) : (
                ...
                <TableRow key={q.id}>
                  ...
                  <TableCell>
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/supplier/quotations/${q.id}`}>مشاهده</Link>
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        ...
        <div className="flex gap-2">
          {page > 0 ? (
            <Button asChild variant="outline" size="sm">
              <Link href={`/supplier/quotations?status=${statusFilter ?? ""}&page=${page - 1}`}>
                قبلی
              </Link>
            </Button>
          ) : null}
