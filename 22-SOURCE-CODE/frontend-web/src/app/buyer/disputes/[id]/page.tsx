...
import { BuyerDisputeActions } from "./buyer-dispute-actions";

interface PageProps {
  params: Promise<{ id: string }>;
}

export default async function BuyerDisputeDetailPage({ params }: PageProps) {
  const { id } = await params;
  const detail = await getDispute(id, "buyer");
  if (!detail) notFound();

  const { dispute } = detail;
  const status = dispute.status;
  const canSubmit = status === "opened" || status === "under_review";
  const canWithdraw = status === "opened" || status === "under_review";
      ...
      {!canSubmit && !canWithdraw ? (
        <Card>
          <CardContent className="p-6 text-sm text-muted-foreground">
            پرونده در وضعیت <Badge variant="outline">{dispute.status}</Badge> قفل‌شده است.
          </CardContent>
        </Card>
      ) : null}
    </div>
