...
export default async function AdminBookingDetailPage({ params }: PageProps) {
  ...
  const detail = await getBooking(id, "admin");
  if (!detail) notFound();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">جزئیات رزرو — ادمین</h1>
          <p className="text-sm text-muted-foreground">
            نمای فقط-خواندنی. اقدام ادمین فقط لغو در حالت‌های غیر-نهایی است.
          </p>
        </div>
        <Button asChild variant="outline" size="sm">
          <Link href="/admin/bookings">بازگشت</Link>
        </Button>
      </div>

      <BookingSummaryCard detail={detail} />
      <BookingActionButtons bookingId={id} audience="admin" status={detail.booking.status} />
