...
import { loadSupplierExecutiveDashboard } from "@/lib/executive/load-supplier-executive-dashboard";
import { getProfile } from "@/lib/auth/get-profile";

export default async function SupplierExecutiveDashboardPage() {
  const profile = await getProfile();
  const bundle = await loadSupplierExecutiveDashboard({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">داشبورد اجرایی — تأمین‌کننده</h1>
        <p className="text-sm text-muted-foreground">
          نمای فقط-خواندنی وضعیت تأمین‌کننده. دعوت‌نامه‌ها، پیشنهادها، قراردادها، شیپمنت‌ها، تسویه‌ها، اختلافات، KYB و اعلان‌ها.
        </p>
      </div>
