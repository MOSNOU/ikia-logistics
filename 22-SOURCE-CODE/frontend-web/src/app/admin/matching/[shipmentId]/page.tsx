...
export default async function AdminMatchingDetailPage({ params }: PageProps) {
  const { shipmentId } = await params;
  const detail = await getShipment(shipmentId, "admin");
  if (!detail) notFound();
  const s = detail.shipment;
  const result = await findMatchingForShipment(shipmentId, { limit: 50 });

  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">تطبیق محموله — ادمین</h1>
          <p className="text-sm text-muted-foreground">
            نمای ادمین. توصیه فقط-خواندنی؛ بدون اختصاص، رزرو یا تغییر محموله.
          </p>
        </div>
        <div className="flex gap-2">
          <Button asChild variant="outline" size="sm">
            <Link href="/admin/matching">بازگشت</Link>
          </Button>
          <Button asChild variant="outline" size="sm">
            <Link href={`/admin/shipments/${shipmentId}`}>محموله</Link>
          </Button>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="p-6 grid gap-3 md:grid-cols-4 text-sm">
          ...
          <div>
            ...
            <div className="font-mono text-xs">{s.organization_id}</div>
          </div>
          <div>
