...
export default async function BuyerFinanceSettlementsPage({ searchParams }: PageProps) {
  const { status, page: pageParam } = await searchParams;
  const page = Math.max(0, Number.parseInt(pageParam ?? "0", 10) || 0);
  const { rows, pageSize } = await listBuyerSettlements({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">تسویه‌ها — نمای مالی</h1>
          <p className="text-sm text-muted-foreground">
            نمای فقط-خواندنی تسویه‌ها از منظر مالی. برای عملیات لیست عمومی به{" "}
            <Link className="underline" href="/buyer/settlements">/buyer/settlements</Link> مراجعه کنید.
          </p>
        </div>
        <Button asChild variant="outline" size="sm">
          <Link href="/buyer/finance">داشبورد مالی</Link>
        </Button>
      </div>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell><AmountCell value={r.held_amount} currency={r.currency} /></TableCell>
                  ...
                  <TableCell>
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/buyer/finance/settlements/${r.id}`}>مشاهده</Link>
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div className="flex justify-between text-xs text-muted-foreground">
        ...
        <div className="flex gap-2">
          {page > 0 ? (
            <Button asChild variant="outline" size="sm">
              <Link href={`/buyer/finance/settlements?status=${status ?? ""}&page=${page - 1}`}>قبلی</Link>
            </Button>
          ) : null}
          {rows.length === pageSize ? (
            <Button asChild variant="outline" size="sm">
              <Link href={`/buyer/finance/settlements?status=${status ?? ""}&page=${page + 1}`}>بعدی</Link>
            </Button>
          ) : null}
        </div>
      </div>
