...
import {
  buyerSubmitEvidence,
  buyerWithdrawDispute,
  type DisputeActionState,
} from "@/lib/dispute/buyer-actions";
...
export function BuyerDisputeActions({
  disputeId,
  canSubmit,
  canWithdraw,
}: {
  ...
  const [submitState, submitAction, submitPending] =
    useActionState<DisputeActionState | null, FormData>(buyerSubmitEvidence, null);
  const [withdrawState, withdrawAction, withdrawPending] =
    useActionState<DisputeActionState | null, FormData>(buyerWithdrawDispute, null);
