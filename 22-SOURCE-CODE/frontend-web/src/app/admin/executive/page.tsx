...
import { loadAdminExecutiveDashboard } from "@/lib/executive/load-admin-executive-dashboard";

export default async function AdminExecutiveDashboardPage() {
  const bundle = await loadAdminExecutiveDashboard();

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">داشبورد اجرایی — ادمین پلتفرم</h1>
        <p className="text-sm text-muted-foreground">
          نمای فقط-خواندنی وضعیت پلتفرم. داده‌ها از سرویس‌های موجود (RFQ، پیشنهاد، ارزیابی، قرارداد، شیپمنت، مالی، تسویه، اختلاف، KYC، اعلان‌ها) جمع‌بندی می‌شوند.
        </p>
      </div>
