...
export default async function AdminSettlementsPage({ searchParams }: PageProps) {
  ...
  const { rows, pageSize } = await listAdminSettlements({ status: statusFilter, page });

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">صف تسویه‌ها (مدیریت)</h1>
        <p className="text-sm text-muted-foreground">
          نمای پلتفرمی روی همه تسویه‌ها برای پایش و اقدام اضطراری.
        </p>
      </div>
                ...
                <TableRow key={s.id}>
                  ...
                  <TableCell>
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/admin/settlements/${s.id}`}>مشاهده</Link>
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        ...
        <div className="flex gap-2">
          {page > 0 ? (
            <Button asChild variant="outline" size="sm">
              <Link href={`/admin/settlements?status=${statusFilter ?? ""}&page=${page - 1}`}>
                قبلی
              </Link>
            </Button>
          ) : null}
