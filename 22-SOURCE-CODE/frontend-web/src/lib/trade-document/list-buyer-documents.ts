...
export interface ListBuyerDocumentsParams {
...
export interface ListBuyerDocumentsResult {
...
export async function listBuyerTradeDocuments({
  documentKind = null,
  documentStatus = null,
  shipmentId = null,
  page = 0,
  pageSize = 25,
}: ListBuyerDocumentsParams = {}): Promise<ListBuyerDocumentsResult> {
  ...
  if (error) {
    console.error("buyer list trade documents", error);
...
export async function getBuyerTradeDocument(
  documentId: string,
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.error("buyer get trade document", error);
    return null;
  }
